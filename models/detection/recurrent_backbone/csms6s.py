import torch
# Bidirectional Channel Scan
class CrossScan_2c(torch.autograd.Function):
    @staticmethod
...
class CrossMerge_2c(torch.autograd.Function):
    @staticmethod
...
# Sparse SS2D
class CrossScan_2hs1ts_pure(torch.autograd.Function):
    @staticmethod
    def forward(ctx, x: torch.Tensor, indices: torch.Tensor):
        B, C, L = x.shape
        ...
        xs[:, 0] = x
        indices = indices.repeat(1, x.shape[1],1)
        assert indices.shape == x.shape
        xs[:, 1] = torch.flip(x.gather(dim=2, index=indices), dims=[-1])
    ...
    def backward(ctx, ys: torch.Tensor):
        B, C, L = ctx.shape
        indices = ctx.indices
        restored_indices = torch.argsort(indices, dim=2)
        y = ys[:, 0] + ys[:, 2].flip(dims=[-1]).view(B, -1, L)
        restored_indices = restored_indices.repeat(1, y.shape[1], 1)
        assert restored_indices.shape == ys[:, 1].flip(dims=[-1]).shape
        y = y + ys[:, 1].flip(dims=[-1]).gather(dim=2, index=restored_indices)
        return y, None

class CrossMerge_2hs1ts_pure(torch.autograd.Function):
    @staticmethod
    def forward(ctx, ys: torch.Tensor, indices: torch.Tensor):
        indices.requires_grad==False
        B, K, D, L = ys.shape
        ctx.indices = indices
        restored_indices = torch.argsort(indices, dim=2)
        y = ys[:, 0] + ys[:, 2].flip(dims=[-1]).view(B, D, -1)
        restored_indices = restored_indices.repeat(1, y.shape[1], 1)
        assert restored_indices.shape == ys[:, 1].flip(dims=[-1]).shape
        y = y + ys[:, 1].flip(dims=[-1]).gather(dim=2, index=restored_indices)
        return y
    ...
    def backward(ctx, x: torch.Tensor):
        ...
        indices = indices.repeat(1, x.shape[1], 1)
        assert indices.shape == x.shape
        xs[:, 1] = torch.flip(x.gather(dim=2, index=indices).contiguous(), dims=[-1])
        xs[:, 2] = torch.flip(xs[:, 0], dims=[-1])
        xs = xs.view(B, 3, C, L)
        return xs, None
...
class SelectiveScanOflexs6(torch.autograd.Function):
    @staticmethod
    @torch.cuda.amp.custom_fwd
    def forward(ctx, u, delta, A, B, C, D=None, delta_bias=None, delta_softplus=False, nrows=1, backnrows=1, oflex=True):
        ctx.delta_softplus = delta_softplus
