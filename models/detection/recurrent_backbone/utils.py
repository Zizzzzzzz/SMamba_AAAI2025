...
try:
    ...
    from .csms6s import CrossScan_2c, CrossMerge_2c, CrossScan_2hs1ts_pure, CrossMerge_2hs1ts_pure
...
class SS2Dv2:
    def __initv2__(
        ...
        **kwargs,    
        ...
        FORWARD_TYPES = dict(
            ...
            v05=partial(self.forward_corev2, force_fp32=False, SelectiveScan=SelectiveScanOflex, no_einsum=True, CrossScan=CrossScan_2hs1ts_pure, CrossMerge=CrossMerge_2hs1ts_pure),
            # # ===============================
    ...
    def forward_corev2(
        ...
        **kwargs,
        ...
        if cascade2d:
            ...
            XX = x.clone()
            index_token = index_token.repeat(1, x.shape[1], 1)
            x = x.flatten(2,3).gather(dim=2, index=index_token)
            xs = CrossScan.apply(x, indices)
            ...
            y: torch.Tensor = CrossMerge.apply(ys, indices)

            XX = XX.to(y.dtype).flatten(2,3).scatter_(2, index_token, y)
    ...
    def forwardv2(self, x: torch.Tensor, index_token: torch.Tensor, indices: torch.Tensor, **kwargs):
        x = self.in_proj(x)
        ...
        x = self.act(x)
        y = self.forward_core(x, index_token, indices)
        y = self.out_act(y)
        if not self.disable_z:
            y = y * z
...
class SS2Dvcm:
    def __initvcm__(
        ...
        **kwargs,    
        ...
        if idx==0:
            self.maxpl = nn.Sequential(nn.MaxPool2d(kernel_size=3, stride=2, padding=1), nn.MaxPool2d(kernel_size=3, stride=2, padding=1),)
        elif idx==1:
            self.maxpl = nn.MaxPool2d(kernel_size=3, stride=2, padding=1)

        self.idx = idx
        factory_kwargs = {"device": None, "dtype": None}
        super().__init__()
        d_inner = int(ssm_ratio * d_model)
        dt_rank = math.ceil(d_ssm / 16) if dt_rank == "auto" else dt_rank
        ...
        FORWARD_TYPES = dict(
            ...
            # v03=partial(self.forward_corev2, force_fp32=(not self.disable_force32), SelectiveScan=SelectiveScanOflex, CrossScan=CrossScanTriton, CrossMerge=CrossMergeTriton),
            # v04=partial(self.forward_corev2, force_fp32=False, SelectiveScan=SelectiveScanOflex, CrossScan=CrossScanTriton, CrossMerge=CrossMergeTriton),
            vcm=partial(self.forward_corev2, force_fp32=False, SelectiveScan=SelectiveScanOflex, no_einsum=True, CrossScan=CrossScan_2c, CrossMerge=CrossMerge_2c),
            # # ===============================
            # v051d=partial(self.forward_corev2, force_fp32=False, SelectiveScan=SelectiveScanOflex, no_einsum=True, CrossScan=getCSM(1)[0], CrossMerge=getCSM(1)[1],
            # ),
            # v052d=partial(self.forward_corev2, force_fp32=False, SelectiveScan=SelectiveScanOflex, no_einsum=True, CrossScan=getCSM(2)[0], CrossMerge=getCSM(2)[1],
        ...
        # x proj ============================
        self.x_proj = [
            nn.Linear(d_ssm, (dt_rank + d_state * 2), bias=False)
            for _ in range(k_group)
        ]
        self.x_proj_weight = nn.Parameter(torch.stack([t.weight for t in self.x_proj], dim=0)) # (K, N, inner)
        del self.x_proj
        ...
        if initialize in ["v0"]:
            # dt proj ============================
            self.dt_projs = [
                self.dt_init(dt_rank, d_ssm, dt_scale, dt_init, dt_min, dt_max, dt_init_floor)
                for _ in range(k_group)
            ]
            self.dt_projs_weight = nn.Parameter(torch.stack([t.weight for t in self.dt_projs], dim=0)) # (K, inner, rank)
            self.dt_projs_bias = nn.Parameter(torch.stack([t.bias for t in self.dt_projs], dim=0)) # (K, inner)
            ...
            # A, D =======================================
            self.A_logs = self.A_log_init(d_state, d_ssm, copies=k_group, merge=True) # (K * D, N)
            self.Ds = self.D_init(d_ssm, copies=k_group, merge=True) # (K * D)
        elif initialize in ["v1"]:
            # simple init dt_projs, A_logs, Ds
            self.Ds = nn.Parameter(torch.ones((k_group * d_inner)))
            self.A_logs = nn.Parameter(torch.randn((k_group * d_inner, d_state))) # A == -A_logs.exp() < 0; # 0 < exp(A * dt) < 1
    ...
    def forwardv2(self, x: torch.Tensor, index_token: torch.Tensor, indices: torch.Tensor, **kwargs):
        B, C, H, W = x.shape
        XX = x.clone()
        index_token2 = index_token.repeat(1, x.shape[1], 1)
        assert index_token2.shape[1] == x.shape[1]
        x = x.flatten(2,3).gather(dim=2, index=index_token2)
        x = x.permute(0, 2, 1).contiguous()

        x = self.in_proj(x)
        x = x.permute(0, 2, 1).contiguous()
        assert index_token2.shape[1] == x.shape[1]
        x = XX.to(x.dtype).flatten(2,3).scatter_(2, index_token2, x).view(B, -1, H, W)
        ...
        if self.with_dconv:
            x = self.conv2d(x) # (b, d, h, w)
        x = self.act(x)
        y = self.forward_core(x, indices)
        y = self.out_act(y)
        if not self.disable_z:
            y = y * z
        XX = y.clone()
        assert index_token2.shape[1] == y.shape[1]
        y = y.flatten(2,3).gather(dim=2, index=index_token2)
        y = y.permute(0, 2, 1).contiguous()
        y = self.out_proj(y)
        y = y.permute(0, 2, 1).contiguous()
        assert index_token2.shape[1] == y.shape[1]
        y = XX.to(x.dtype).flatten(2,3).scatter_(2, index_token2, y).view(B, -1, H, W)
        out = self.dropout(y)
        return out
class SparseSS2D(nn.Module, mamba_init, SS2Dv2):
    def __init__(
        self,
        # basic dims ===========
        d_model=96,
...
class SS2D_cm(nn.Module, mamba_init, SS2Dvcm):
    def __init__(
        self,
        idx=0,
        # basic dims ===========
        ...
        **kwargs,
        ...
        super().__init__()
        kwargs.update(
            idx=idx, d_model=d_model, d_state=d_state, ssm_ratio=ssm_ratio, dt_rank=dt_rank,
            act_layer=act_layer, d_conv=d_conv, conv_bias=conv_bias, dropout=dropout, bias=bias,
            dt_min=dt_min, dt_max=dt_max, dt_init=dt_init, dt_scale=dt_scale, dt_init_floor=dt_init_floor,
            initialize=initialize, forward_type=forward_type, channel_first=channel_first,
        )
        self.__initvcm__(**kwargs)
...
class SCMM(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__()
        # # =========================
        self.spas = [int((hw[0]/16)*(hw[1]/16)), int((hw[0]/32)*(hw[1]/32))]
        self.dims=[64, 128, 256, 512]
        self.ssm_branch = ssm_ratio > 0
        self.gci_branch = mlp_ratio > 0
        self.use_checkpoint = use_checkpoint
        self.post_norm = post_norm

        hidden_dim = self.dims[idx]
        hidden_dim2 = self.spas[idx-2]
        ...
        if self.gci_branch:
            ...
            self.op2 = SS2D_cm(
                ...
                initialize=ssm_init,
                # ==========================
                forward_type="vcm_noz",
                channel_first=channel_first,
            )

    def _forward(self, input: torch.Tensor):
        x, index_token, indices = input
        if self.ssm_branch:
            if self.post_norm:
                x = x + self.drop_path(self.norm(self.op(x)))
            else:
                xx = self.op(self.norm(x), index_token, indices)
                xx = xx * self.sa(xx)
        ...
        if self.gci_branch:
            if self.post_norm:
                ...
                xx = self.op2(self.norm2(x), index_token, indices) + self.conv(x)
                xx = xx * self.ca(xx)
                x = x + self.drop_path(xx)
        return (x, index_token, indices)
    ...
    def forward(self, input: torch.Tensor):
        ...
        if self.use_checkpoint:
            ...
            return self._forward((x, index_token, indices))
    ...
    def _forward(self, input: torch.Tensor):
        ...
        if self.mlp_branch:
            if self.post_norm:
                ...
                index_token2 = index_token.repeat(1, xx.shape[1], 1)
                assert index_token2.shape[1] == xx.shape[1]
                xx = xx.flatten(2,3).gather(dim=2, index=index_token2)
                xx = xx.permute(0, 2, 1).contiguous()

                xx = self.mlp(xx)
                xx = xx.permute(0, 2, 1).contiguous()
                assert index_token2.shape[1] == xx.shape[1]
                xx = XX.to(x.dtype).flatten(2,3).scatter_(2, index_token2, xx.to(x.dtype)).view(B, -1, H, W)
