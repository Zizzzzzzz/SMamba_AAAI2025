...
def evaluate_list(result_boxes_list,
    ...
    assert camera in {'gen1', 'gen4', 'etram'}

    if camera == 'gen1':
        classes = ("car", "pedestrian")
    elif camera == 'gen4' or camera == 'etram':
        classes = ("pedestrian", "two-wheeler", "car")
    # elif camera == 'etram':
    #     classes = ("pedestrian", "two-wheeler", "car") #("pedestrian", "car", "two-wheeler")
    else:
    ...
    if apply_bbox_filters:
        ...
        min_box_diag = 60 if camera == 'gen4' or camera == 'etram' else 30
        # In the supplementary mat, they say that min_box_side is 20 for gen4.
        min_box_side = 20 if camera == 'gen4' or camera == 'etram' else 10
        if downsampled_by_2:
