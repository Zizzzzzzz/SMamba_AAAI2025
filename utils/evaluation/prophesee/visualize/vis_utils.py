...
LABELMAP_eTram_SHORT = ('pedestrian', 'two wheeler', 'car') #('pedestrian', 'car', 'two wheeler')
...
def draw_bboxes_bbv(img, boxes, labelmap=LABELMAP_GEN1) -> np.ndarray:
    ...
    if labelmap == LABELMAP_GEN1:
        ...
        scale_multiplier = 4
    elif labelmap == LABELMAP_GEN4_SHORT:
        ...
        assert labelmap == LABELMAP_eTram_SHORT
        classid2colors = {
            0: (0, 0, 255),  # ped -> blue (rgb)
            1: (0, 255, 255),  # car cyan (rgb)
            2: (255, 255, 0),  # 2-wheeler -> yellow (rgb)
        }
        scale_multiplier = 2
